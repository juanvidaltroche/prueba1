...
acl.application = {
  init:function(){
    storeUserProcess = function (n, r, i) {
      ...
      myMask.show();
      Ext.Ajax.request({
        url: "../servicios/blpReporteDiarioTurnosAjax.php",
        method: "POST",
  ...
  var storeGridTotal = new Ext.data.GroupingStore({
    ...
    var storeRecaudacionPeriodica = new Ext.data.GroupingStore({
		...
		groupField: 'LQD_TURNO',
		sortInfo: {field:"LQD_TURNO", direction:"ASC"},  
  ...
  var sHoraInicio = new Ext.form.TimeField({  
  id:'sHoraInicio',
  emptyText:'Inserte hora ...',
  fieldLabel:'Time',
  format: 'H:i',
  //minValue: '00:00',
  //maxValue: '23:00',
  ...
  var sHoraFinal = new Ext.form.TimeField({  
  id:'sHoraFinal',
  emptyText:'Inserte hora ...',
  fieldLabel:'Time',
  format: 'H:i',
  ...
  var cboTurnos = new Ext.form.ComboBox({
      id: 'sIdTurnos',
      name: 'sIdTurnos',
      mode: "local",
      triggerAction: "all",
...
//      hiddenName: 'turno',
    ...
    var btnPrm = new Ext.Action({
      ...
      handler: function() {
        ...
        var sTitulos= "REPORTE DE RECAUDACION POR CAJERO |" + "DE: "+fechaIni+ "|"+" HASTA: " + fechaFin + "| MONTO RECAUDADO: | Bs. " + storeGridTotal.data.items[0].data['MONTO'] + "| SOBRANTE: | Bs." + storeGridTotal.data.items[0].data['SOBRANTE_FALTANTE'] + " | MONTO COBRADO:   | Bs. "+ storeGridTotal.data.items[0].data['MONTO_COBRADO'];
	...
	var btnBuscarFecha = new Ext.Action({
      ...
      handler: function() {			  
        ...
        var sHoraInicio = Ext.getCmp('sHoraInicio').getValue();
        var sHoraFinal = Ext.getCmp('sHoraFinal').getValue(); 
        var sTurno = Ext.getCmp('sIdTurnos').getValue();
        //alert(sTurno);

		storeRecaudacionPeriodica.reload({ params: {option: "LIST_2", pageSize: pageSize, fechaInicio: sFechaInicio, fechaFinal: sFechaFinal, horaInicio: sHoraInicio, horaFinal: sHoraFinal, anfitrion: sIdAnfitrionI, turno: sTurno}}); 
    storeGridTotal.reload({ params: {"option": "LST_MONTO2", "pageSize": 1000, fechaInicio: sFechaInicio, fechaFinal: sFechaFinal, horaInicio: sHoraInicio, horaFinal: sHoraFinal, anfitrion: sIdAnfitrionI, turno: sTurno}});
    ...
    var cmodel = new Ext.grid.ColumnModel({
      ...
      columns:[
         ...
         {header: "TURNO", dataIndex: "LQD_TURNO", align: "right", hidden: false, hideable: false},
         ...
         {header: "SOBRANTE", dataIndex: "SOBRANTE_FALTANTE", summaryType: 'sum', align: "right", hidden: false, hideable: false, 
                 renderer: function(v, params, data){                 
                    if (v<0)
    ...
    var grdRecaudacion = new Ext.grid.GridPanel({
		...
		plugins: summary,
		tbar: [' FECHA INICIO: ',sFechaInicio, 'FECHA FINAL: ',sFechaFinal, 'HORA INICIO: ',sHoraInicio, 'HORA FINAL: ',sHoraFinal, 'INGRESAR NOMBRE: ', cboAnfitrion, cboTurnos, btnBuscarFecha, ' ' , btnPrm],
		bbar: pagingUser,
