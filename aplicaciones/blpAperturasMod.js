...
var LABEL_TITLE_PANEL_1 = "..:: MODIFICACION DE LIQUIDACIONES MANUALES ::..";
...
acl.application = {
    init: function () {
        ...
        storeFuncionarioProcess = function (n, r, i) {
            ...
            //AJAX FUNCIONARIOS
            Ext.Ajax.request({
                url: "../servicios/blpAperturasModAjax.php",
                method: "POST",
                params: {
                    "option": "LST",
                    "vFechaInicio": DESDE,
...
//        maskRe: /[0-9,-]/
        var peNumberTest = /[0-9,-]\-[0-9,-]\-[0-9,-]\-[0-9]/;
        ...
        newForm = new Ext.FormPanel({
            ...
            items: [
                {
                    ...
                    fieldLabel: '<B>TURNO DEL ANFITRION</B>',
                    id: 'LABEL_TURNO',
                    name: 'LABEL_TURNO',
                    allowBlank: false,
                    xtype: 'combo',
                    ...
                    listeners: {
                        select: function(combo, record, index) {
                          ...
                          if (nro2.length==1){
                            nroFinal = nro1 + "-00" + nro2 + "-" ;
                          } else if (nro2.length==2){
                            nroFinal = nro1 + "-0" + nro2 + "-" ;
                          } else {
                            nroFinal = nro1 + "-" + nro2 + "-" ;
                          }
                ...
                },
                {
                        ...
                        listeners: {check: function(){
                            ...
                            if (cb1 == true) {
                                ...
                                nro2 = Ext.getCmp('LABEL_ESTCIVIL').getValue();
                                if (nro2.length==1){
                                    nroFinal = nro1 + "-00" + nro2 + "-00-00" ;
                                  } else if (nro2.length==2){
                                    nroFinal = nro1 + "-0" + nro2 + "-00-00" ;
                                  } else {
                ...
                ,{
                  xtype: 'box',
                  autoEl: {cn: '<center><font color="#FF0000">Ejemplo de Nro. de seguridad:  <b> 11-079-23-04 </b></font></center>'}
                }
                ,{
                    xtype: 'textfield',
                    //vtype: 'peNumber',
            ...
            buttons: [{
                ...
                text: 'GRABAR',
                handler: function () {
                    Ext.Msg.confirm('Cobrando Pasajes', 'Usted cobrara: Bs. ' +  Ext.getCmp('LABEL_CI').getValue() + '<br> Esta seguro de querer hacer esto?',function(btn){  
                              //PANCHITO
                              var res = Ext.getCmp('LABEL_MAGICO').getValue();
                              sCadena = res.split('-');
                              if(btn === 'yes'){  
                                if (sCadena.length==4 && sCadena[0] >= 0 && sCadena[1] >= 0 && sCadena[2] >= 0&& sCadena[3] >= 0) {
                                    //desde
                                    var monto = parseInt(Ext.getCmp('LABEL_CI').getValue());
                                    if (monto > 0){
                                      if (newForm.getForm().isValid() ) {
                                        form_action = 1; 
                                        if (sw == 0) {
                                            newForm.getForm().submit({
                                                url: '../servicios/blpAperturasModAjax.php?option=NEW',
                                            ...
                                            var value = rec.get('iddd');
                                            newForm.getForm().submit({
                                                url: '../servicios/blpAperturasModAjax.php?option=UPD',
                                                method: "POST",
                                                params: {
                                                    "option": "UPD",
                                                    "i": value
                                        ...
                                        Ext.MessageBox.alert("Advertencia", "Monto recaudado debe ser mayor a Bs. 0.00");
                                }
                                    //hasta aca
                                } else {
                                        Ext.MessageBox.alert("Advertencia", "Nro de Seguridad debe tener 4 cifras");
                                }
                              }     
                          });
                }
        ...
        var btnUpt = new Ext.Action({
            ...
            handler: function onUpdate() {
                ...
                winForm.title = 'MODIFICACION DE REGISTRO';
                doCargaFormulario(rec);
            }
        });
        function doCargaFormulario(record) {
            Ext.getCmp('LABEL_TURNO').setValue(record.get('LQD_TURNO'));
            Ext.getCmp('LABEL_EXPEDIDO').setValue(record.get('LQD_ANFITRION'));
            Ext.getCmp('LABEL_ESTCIVIL').setValue(record.get('LQD_TIPO_HERRAMIENTA_ID'));
            
            Ext.getCmp('LABEL_MAGICO').setValue(record.get('LQD_NRO_MAGICO'));
            Ext.getCmp('LABEL_PREFERENCIALDIURNO').setValue(record.get('LQD_PD'));
            Ext.getCmp('LABEL_NORMALDIURNO').setValue(record.get('LQD_ND'));
            Ext.getCmp('LABEL_NORMALNOCTURNO').setValue(record.get('LQD_NN'));
            Ext.getCmp('LABEL_PREFERENCIALNOCTURNO').setValue(record.get('LQD_PN'));
            Ext.getCmp('LABEL_NOMBRE').setValue(record.get('LQD_MONTO'));
            Ext.getCmp('LABEL_PATERNO').setValue(record.get('LQD_MONTOSF'));
            res1 = parseFloat(record.get('LQD_MONTO')) + parseFloat(record.get('LQD_MONTOSF'));
            Ext.getCmp('LABEL_CI').setValue(parseFloat(res1).toFixed(2));
            if(record.get('RUTA_DESC_GRILLA') == "COTAHUMA") {
             montoPasaje = 1.80;
             Ext.getCmp('LABEL_RUTA').setValue(1);
            }
            if(record.get('RUTA_DESC_GRILLA') == "SAN ANTONIO"){
              montoPasaje = 1.80;
              Ext.getCmp('LABEL_RUTA').setValue(2);

            } 
            if(record.get('RUTA_DESC_GRILLA') == "ZONA SUR"){
              montoPasaje = 2;
              Ext.getCmp('LABEL_RUTA').setValue(3);

            }
            Ext.getCmp('LABEL_CAJERO').setValue(record.get('LQD_CAJERO'));
    ...
    var btnDel = new Ext.Action({
        ...
        storeFuncionario.on('load', function (s, r, p) {
            storeFuncionario.each(function (a, i) {
                Ext.fly(grdpnlUser.getView().getRow(i)).addClass('green');
                Ext.fly(grdpnlUser.getView().getRow(i)).setStyle({ 'border':'2px solid #000'});
                return false;
            })
        }, this);
