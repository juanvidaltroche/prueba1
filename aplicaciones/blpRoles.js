...
acl.application = {
  init:function(){
    storeProcess = function (n, r, i) {
	  ...
	  //AJAX USUARIOS
      Ext.Ajax.request({
        url: "../servicios/blpRolesAjax.php",
        method: "POST",
        params: {"option": "LST", "pageSize": n, "limit": r, "start": i},

        success:function (result, request) {
                  storeRoles.loadData(Ext.util.JSON.decode(result.responseText));
                  myMask.hide();
                },
	...
	var storeRoles = new Ext.data.GroupingStore({
        ...
        reader:new Ext.data.JsonReader({
            ...
            fields: [{name: "ROL_ID", allowBlank: false},
                     {name: "ROL_DESCRIPCION", allowBlank: false},
					 {name: "ROL_REGISTRADO", allowBlank: false},
					 {name: "ROL_MODIFICADO", allowBlank: false},
					 {name: "ROL_USUARIO", allowBlank: false},
					 {name: "ROL_ESTADO", allowBlank: false},
    ...
    newForm = new Ext.FormPanel({
                ...
                items: [					
					...
					{
                        fieldLabel: 'ROL',
                        id: 'ROL_DESCRIPCION_LABEL',
                        name: 'ROL_DESCRIPCION_LABEL',
                        allowBlank:false
                    },				
					{
                        fieldLabel: 'ESTADO',
                        id: 'ROL_ESTADO_COMBO',
                        name: 'ROL_ESTADO_COMBO',
                        allowBlank: false,
						xtype:          'combo',
						mode:           'local',
						value:          'ACTIVO',
                    ...
                    handler: function() {
                        if(newForm.getForm().isValid()){
							...
							if(sw =="0")
							{
                                ...
                                url: '../servicios/blpRolesAjax.php?option=NEW',
    ...
    var btnNew = new Ext.Action({
	  ...
	  iconCls: 'icon-add',
      handler: function() {
        Ext.getCmp('ROL_DESCRIPCION_LABEL').reset(); 	
		Ext.getCmp('ROL_ESTADO_COMBO').reset();			
		sw=0;		
	...
	var btnUpd = new Ext.Action({
      ...
      handler:     function onUpdate() {
		...
		Ext.getCmp('ROL_DESCRIPCION_LABEL').setValue(rec.get('ROL_DESCRIPCION'));  
		Ext.getCmp('ROL_ESTADO_COMBO').setValue(rec.get('ROL_ESTADO'));			
    ...
    var cmodel = new Ext.grid.ColumnModel({
      ...
      columns:[
			   {id:"ID",header: "ID", dataIndex: "ROL_ID",hidden: false, hideable: true, width: 50},
               {header: "ROL",dataIndex: "ROL_DESCRIPCION", align: "left", hidden: false, hideable: true, editor: new Ext.form.TextField({})},			   
			   {header: "REGISTRADO", dataIndex: "ROL_REGISTRADO", align: "left", hidden: false, hideable: true, editor: new Ext.form.TextField({})},
			   {header: "MODIFICADO",dataIndex: "ROL_MODIFICADO", align: "left", hidden: false, hideable: true, editor: new Ext.form.TextField({})},
			   {header: "USUARIO",dataIndex: "ROL_USUARIO", align: "left", hidden: false, hideable: true, editor: new Ext.form.TextField({})},			   
			   {header: "ESTADO",dataIndex: "ROL_ESTADO", align: "left", hidden: false, hideable: true, editor: new Ext.form.TextField({}),			   
