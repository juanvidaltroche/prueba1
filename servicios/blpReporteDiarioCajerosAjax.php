...
function rowsGet($r, $i) {
    ...
    $sSQL = "SELECT LQD_ID, CONCAT(M1.FUNCIONARIO_PATERNO, ' ', M1.FUNCIONARIO_MATERNO, ' ', M1.FUNCIONARIO_NOMBRES ) AS ANFITRION_NOMBRE_COMPLETO,
                ...
                INNER JOIN MB_FUNCIONARIOS M2 ON M2.FUNCIONARIO_ID = U2.USUARIO_FUNCIONARIO_ID
...
function rowsTotal2($r, $i, $fIni, $fFin, $sAnfitrion) {
    if ($sAnfitrion=='')
        $sSQL = "SELECT SUM(CAST(LQD_MONTO AS DECIMAL(20,2))) AS MONTO, SUM(CAST(LQD_MONTOSF AS DECIMAL(20,2))) AS SOBRANTE_FALTANTE,
                (SUM(CAST(LQD_MONTO AS DECIMAL(20,2))) + SUM(CAST(LQD_MONTOSF AS DECIMAL(20,2)))) AS MONTO_COBRADO
                FROM MB_LIQ_MANUALES
                INNER JOIN SA_USUARIOS U1 ON LQD_CAJERO= U1.USUARIO_ID
                INNER JOIN MB_FUNCIONARIOS M1 ON M1.FUNCIONARIO_ID = U1.USUARIO_FUNCIONARIO_ID  
                WHERE LQD_ESTADO = 'ACTIVO' = 'ACTIVO' AND LQD_REGISTRO BETWEEN '$fIni' AND '$fFin' ";
    else
        $sSQL = "SELECT SUM(CAST(LQD_MONTO AS DECIMAL(20,2))) AS MONTO, SUM(CAST(LQD_MONTOSF AS DECIMAL(20,2))) AS SOBRANTE_FALTANTE,
                (SUM(CAST(LQD_MONTO AS DECIMAL(20,2))) + SUM(CAST(LQD_MONTOSF AS DECIMAL(20,2)))) AS MONTO_COBRADO
                FROM MB_LIQ_MANUALES
                INNER JOIN SA_USUARIOS U1 ON LQD_CAJERO= U1.USUARIO_ID
                INNER JOIN MB_FUNCIONARIOS M1 ON M1.FUNCIONARIO_ID = U1.USUARIO_FUNCIONARIO_ID  
                WHERE LQD_ESTADO = 'ACTIVO' AND LQD_REGISTRO BETWEEN '$fIni' AND '$fFin' AND FUNCIONARIO_ID = '" . $sAnfitrion . "'";
    $aResult = executeQuery($sSQL, "COBRO_BUSES_ORIGEN");
    return (array(sizeof($aResult), array_slice($aResult, $i, $r)));
}
try {
    ...
    switch ($option) {
		...
		case "LIST_2": 
                    ...
                    list($nLen, $aData) = rowsGet2($limit, $start, $sFechaBusquedaI, $sFechaBusquedaF, $sAnfitrion);
                    echo json_encode(array("success" => true, "resultTotal" => $nLen, "resultRoot" => $aData));
                    break;
        ...
        case "LST_MONTO2": 
                    ...
                    $sFechaBusquedaF = $fFinal[0] . " " . $hFin;
                    $sAnfitrion = isset($_REQUEST["anfitrion"])? $_REQUEST["anfitrion"] : 0;
                    list($nLen, $aData) = rowsTotal2($limit, $start, $sFechaBusquedaI, $sFechaBusquedaF, $sAnfitrion);
                    echo json_encode(array("success" => true, "resultTotal" => $nLen, "resultRoot" => $aData));
                    break;
        case "LST44": 
                    $pageSize = $_REQUEST["pageSize"];
