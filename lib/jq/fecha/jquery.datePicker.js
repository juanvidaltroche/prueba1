...
* Copyright (c) 2008 Bram Van Damme (http://www.bram.us/)
...
(function ($) {
    ...
    $.fn.extend({
        ...
        **/
        datePicker: function (s) {
            ...
            s = $.extend(
				{
				    disallowedDays: undefined, // Bramus!
				    month: undefined,
				    year: undefined,
				    startDate: undefined,
				    endDate: undefined,
    ...
    function DatePicker(ele) {
        ...
        this.disallowedDays = null; // Bramus!
    };
    $.extend(
		DatePicker.prototype,
		{
		    init: function (s) {
		        ...
		        this.disallowedDays = s.disallowedDays; // Bramus!
		        if (this.inline) {
		...
		_renderCalendar: function () {
		    ...
		    /* Begin Bramus! Mod : disallowed weekdays */
		    if (this.disallowedDays) {

		        // scope breaking
		        var that = this;

		        // loop all cells (current month)
		        $('.dp-calendar td.current-month', this.context).each(
						function () {
						    // get the cell
						    var $this = $(this);
						    ...
						    var $thisDate = new Date(that.displayedMonth + 1 + '/' + $this.text() + '/' + that.displayedYear);

						    // get the weekday of the date of the cell
						    var $thisWeekday = $thisDate.getDay();

						    // if the weekday is in disallowed weekdays, then disable it.
						    if ($.inArray($thisWeekday, that.disallowedDays) != -1) {
						        $this.addClass('disabled');
						    }
						}
					);

		        // loop all cells (other month)
		        $('.dp-calendar td.other-month', this.context).each(
						function () {
						    // get the cell
						    var $this = $(this), $month = that.displayedMonth + 1, $year = that.displayedYear;

						    // adjust the date (prev mo or next mo?)
						    if (Number($this.text()) > 15) {
						        $month = $month - 1;
						    } else {
						        $month = $month + 1;
						    }

						    // month < 1? then we're december the year before!
						    if ($month < 1) {
						        $month = 12;
						        $year = $year - 1;
						    }

						    // month > 12? then we're january the year after!
						    if ($month > 12) {
						        $month = 1;
						        $year = $year + 1;
						    }

						    // create the date of the cell
						    var $thisDate = new Date($month + '/' + $this.text() + '/' + $year);
		    ...
		    /* End Bramus! Mod : disallowed weekdays */
